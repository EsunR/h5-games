...
export default class Play extends Laya.Sprite {
    ...
    create() {
        ...
        Laya.stage.on(Event.MOUSE_MOVE, this, (event) => {
            this.moveGem(
                event.stageX - this.x - 28,
                event.stageY - this.y - 21
            );
        });
        this.fillGems();
    }
    ...
    fillGems() {
        Object.keys(gems).forEach((id) => {
            ...
            this.randomColor(gem);
            gem.pos(
                gem.posX * GEM_SIZE + GEM_SIZE / 2,
                gem.posY * GEM_SIZE + GEM_SIZE / 2
            );
            this.addChild(gem);
        });
    }
    ...
    tweenGem(gem, nextX, nextY, count, cb) {
        ...
        Tween.to(
            gem,
            {
                x: nextX * GEM_SIZE + GEM_SIZE / 2,
                y: nextY * GEM_SIZE + GEM_SIZE / 2,
            },
            50 * count,
            Ease.sineOut,
            Handler.create(this, () => {
    ...
    killGem(gem) {
        ...
        pubsub.event(
            "KILL_5",
            Math.floor((waitKill_H.length + waitKill_V.length) / 3)
        );
    ...
    randomColor(gem) {
        ...
        var xColor =
                prev1x &&
                prev2x &&
                prev1x.color == prev2x.color &&
                prev1x.color,
            yColor =
                prev1y &&
                prev2y &&
                prev1y.color == prev2y.color &&
                prev1y.color;
        do {
            gem.color = COLORS[randomNum(COLORS.length - 1)];
